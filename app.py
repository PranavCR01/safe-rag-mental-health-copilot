...
# # --- FIXED: LangChain memory imports for multi-turn context ---
# from langchain_openai import ChatOpenAI

# # Try multiple import paths for ConversationSummaryBufferMemory
# try:
#     from langchain.memory import ConversationSummaryBufferMemory
# except ImportError:
#     try:
...
#     except ImportError:
#         # If still not working, we'll define a simple alternative
...
#         created_at DATETIME DEFAULT CURRENT_TIMESTAMP
#     );""")
#     # ----- schema migration: add HITL columns if missing -----
#     cur = con.cursor()  
#     cur.execute("PRAGMA table_info(chats);")  
#     cols = {r[1] for r in cur.fetchall()}  
#     if "reviewed" not in cols:  
...
# # -------- Per-user memory (LangChain) --------
# # Simple fallback if LangChain memory not available
# class SimpleMemory:
#     """Fallback simple memory if LangChain not available"""
#     def __init__(self):
#         self.history = []
    
#     def load_memory_variables(self, inputs):
#         return {"chat_history": "\n".join(self.history[-5:])}  # Last 5 turns
